<script lang="ts">
    ...
    const showDrink = async () => {
        ...
        try {
            ...
            drinkOptions = drinks.map(drink => ({
                ...
                price: drink.price,
                count: 0,
                value: drink.drink_id // Use drink_id for unique identification
            }));
            console.log('Drink options loaded:', drinkOptions);
        } catch (err) {
    ...
    const changeTable = async () => {
        ...
        try {
            ...
            const {
                data: tables,
                error: tableError
            } = await supabase.from('Table').select('table_name, table_type, table_id');
            const {
                data: tableAvailability,
                error: availabilityError
            } = await supabase.from('Table Availability').select('availability, table_id');
            if (tableError || availabilityError) throw error;
            const availabilityMap = new Map(tableAvailability.map((entry) => [entry.table_id, entry.availability]));
    ...
    const calculateTotal = () => {
        ...
        if (reserveSelected === "1") { // Hourly rate
            ...
            total_price = dailyRate; // should be good
            ...
            total_price = weeklyRate; // same here
        } else {
    ...
    const handleConfirm = async () => {
        ...
        if (toggleReserve) {
            ...
            if (reserveForm) {
                ...
                if (reserveSelected === "1") { // Hourly rate
                    ...
                    weeklyEnd.setDate(weeklyEnd.getDate() + 15); // Move to the next 15 days 6 am
                    weeklyEnd.setHours(6, 0, 0, 0); // Set time to 6:00 AM
                    // Calculate the duration in hours
                    const weeklyDuration = (weeklyEnd.getTime() - start.getTime()) / (1000 * 60 * 60); // Duration in hours
                    const durationWeeklyInterval = `${Math.ceil(weeklyDuration)} hours`; // Round up to nearest hour
                    console.log(customer_id, chosenTable_id, startDate, durationWeeklyInterval, weeklyEnd);
                    // this should tell the customer the end date
                    const {
                        reserveError
                    } = await supabase.from('Table Reservation').insert({
                        customer_id: customer_id,
        ...
        if (toggleOrder) {
            ...
            if (orderForm) {
                ...
                const drinkOrders = totalOrder.map(drink => ({
                    customer_id: customer_id,
                    drink_id: drink.value,
                    receipt_no: receipt_no,
                    qty: drink.count,
                    total_price: drink.count * drink.price
                }));
                console.log("Drink Orders:", drinkOrders);
                try {
                    // Step 1: Create Receipt
                    const total_price = totalOrder.reduce((sum, drink) => sum + drink.count * drink.price, 0);
                    const {
                        data: receiptData,
                        error: receiptError
                    } = await supabase.from('Drink Receipt').insert({
                        receipt_no: receipt_no,
                    ...
                    // Step 3: Create Order Status
                    const {
                        error: statusError
                    } = await supabase.from('Drink Order Status').insert({
                        receipt_no: receipt_no,
                        is_incoming: true,
                        is_ongoing: false,
                        is_done: false,
                    ...
                    if (statusError) {
                        console.error("Error creating order status:", statusError.message);
                        return;
                    }
                    // Insert drink orders into the database
                    console.log(drinkOrders);
                    const {
                        error: orderError
                    } = await supabase.from('Drink Order Line').insert(drinkOrders);
                    if (orderError) {
                        console.error("Error adding drink orders:", orderError.message);
                    } else {
                        console.log("Drink orders added successfully!");
                    }
                } catch (err) {
...
<style>
    ...
    .date-input {
        ...
        /* Match the white background of the card */
        border: 2px solid #38728A;
        /* Match the border color of the card */
        border-radius: 20px;
        /* Rounded corners to match .rounded-3xl */
        padding: 0.5rem 1rem;
        /* Add padding for a nice look */
        width: 100%;
        /* Make it fill the width of the container */
    ...
    .date-input:focus {
        ...
        border-color: #38728A;
        /* Keep the border color as primary when focused */
    ...
    .select-style:focus {
        ...
        border-color: #38728A;
        /* Keep the border color on focus */
    ...
    .select-style option {
        padding: 0.5rem;
        /* Adds spacing to the option text */
    }
</style>
<HideOverflow />
